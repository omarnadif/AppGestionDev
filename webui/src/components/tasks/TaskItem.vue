      ...
      <div class="flex items-center gap-4">
        <div class="flex-grow h-2 bg-slate-700 rounded-full overflow-hidden">
          <div 
            ...
            :class="{
              'bg-orange-500': (task.percentage || getDefaultPercentage) < 30,
              'bg-cyan-500': (task.percentage || getDefaultPercentage) >= 30 && (task.percentage || getDefaultPercentage) < 70,
              'bg-green-500': (task.percentage || getDefaultPercentage) >= 70
            }"
          ></div>
        </div>
        <span class="text-sm text-slate-400 min-w-[3rem]">
          {{ task.percentage || getDefaultPercentage }}%
        </span>
      </div>
